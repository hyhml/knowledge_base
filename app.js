...
const seedKnowledge = [
  {
    ...
    "reviewDate": "",
    "prerequisites": [],
    "explanation": "集合就是把满足同一个条件的对象放在一起。做题时先看元素是什么，再看条件是什么，最后判断元素能不能放进这个集合。",
    "keyPoints": [
  ...
  },
  {
    ...
    "explanation": "判断充分必要条件时，把命题写成“如果 p，那么 q”。只要 p 能推出 q，p 就是 q 的充分条件；反过来 q 能推出 p，p 就是 q 的必要条件。",
    "keyPoints": [
      "p => q：p 是 q 的充分条件。",
      "q => p：p 是 q 的必要条件。",
      "两个方向都能推出，就是充要条件。"
  ...
  },
  {
    ...
    "explanation": "定义域是 x 能取哪些数，值域是 y 最后能得到哪些数。遇到函数题，先不要急着算，第一步通常是检查 x 有没有限制。",
    "keyPoints": [
      "分母不能为 0。",
      "偶次根号内必须大于等于 0。",
      "对数真数必须大于 0。",
  ...
  },
  {
    ...
    "explanation": "单调性看的是 x 变大时，y 是一直变大还是一直变小。基础题可以看图像、看一次函数斜率、看二次函数对称轴，复杂题再用定义或导数。",
    "keyPoints": [
  ...
  },
  {
    ...
    "explanation": "奇偶性先看定义域是否关于 0 对称，再比较 f(-x) 和 f(x)。f(-x)=f(x) 是偶函数，f(-x)=-f(x) 是奇函数。",
    "keyPoints": [
      "判断奇偶性前必须先看定义域。",
      "偶函数图像关于 y 轴对称。",
      "奇函数图像关于原点对称。"
  ...
  },
  {
    ...
    "explanation": "指数和对数是一对反向运算。指数函数看底数是否大于 1；对数函数还要先检查真数大于 0、底数大于 0 且不等于 1。",
    "keyPoints": [
  ...
  },
  {
    ...
    "reviewDate": "",
    "prerequisites": [],
    "explanation": "三角函数的符号由终边所在象限决定。基础薄弱时先记单位圆：cos 看横坐标，sin 看纵坐标，tan 看纵坐标除以横坐标。",
    "keyPoints": [
  ...
  },
  {
    ...
    "explanation": "诱导公式本质是把大角、负角转成熟悉的锐角或特殊角。处理时先确定函数名是否变化，再确定符号。",
    "keyPoints": [
      "sin²x + cos²x = 1。",
      "tan x = sin x / cos x。",
      "kπ ± x 通常看象限符号。"
  ...
  },
  {
    ...
    "reviewDate": "",
    "prerequisites": [],
    "explanation": "通项 an 是第 n 项的表达式，前 n 项和 Sn 是从第 1 项加到第 n 项。看到 Sn 和 an 的关系题，要记住 an=Sn-S(n-1)。",
    "keyPoints": [
      "n>=2 时，an = Sn - S(n-1)。",
      "求出 an 后要单独检查 a1。",
      "数列题里的 n 通常是正整数。"
  ...
  },
  {
    ...
    "explanation": "等差数列的每一项比前一项多同一个数，这个固定差叫公差。题目中出现相邻两项差相同，就优先考虑等差数列。",
    "keyPoints": [
      "通项公式：an=a1+(n-1)d。",
      "前 n 项和：Sn=n(a1+an)/2。",
      "等差中项：2b=a+c。"
  ...
  },
  {
    ...
    "explanation": "等比数列的每一项除以前一项都等于同一个数，这个固定比叫公比。和等差数列不同，它是乘法变化。",
    "keyPoints": [
      "通项公式：an=a1q^(n-1)。",
  ...
  },
  {
    ...
    "reviewDate": "",
    "prerequisites": [],
    "explanation": "不等式最容易出错的是乘除负数。两边同时乘或除以负数时，不等号方向必须改变。",
    "keyPoints": [
      "两边加减同一个数，不等号方向不变。",
      "两边乘除正数，不等号方向不变。",
      "两边乘除负数，不等号方向改变。"
  ...
  },
  {
    ...
    "explanation": "基本不等式常用于求最值，但必须满足正数条件，并且等号能取到。做题时先检查“正、定、等”三个条件。",
    "keyPoints": [
      "a>0，b>0 时，a+b >= 2√ab。",
      "使用前要有正数条件。",
      "求最值时要检查等号成立条件。"
  ...
  },
  {
    ...
    "reviewDate": "",
    "prerequisites": [],
    "explanation": "向量既有大小又有方向。线性运算可以用几何法看平移，也可以用坐标法逐项相加减。",
    "keyPoints": [
      "向量相等要求长度相等且方向相同。",
  ...
  },
  {
    ...
    "explanation": "数量积把两个向量变成一个数。它常用来求夹角、判断垂直，也可以用坐标公式快速计算。",
    "keyPoints": [
      "a·b=|a||b|cosθ。",
  ...
  },
  {
    ...
    "reviewDate": "",
    "prerequisites": [],
    "explanation": "直线题先看能不能求斜率。斜率表示直线倾斜程度，已知一点和斜率时可用点斜式。",
    "keyPoints": [
  ...
  },
  {
    ...
    "explanation": "圆的标准方程直接告诉你圆心和半径。看到 x²、y² 系数相同，可以尝试配方化成标准方程。",
    "keyPoints": [
      "标准方程：(x-a)²+(y-b)²=r²。",
      "圆心是 (a,b)，半径是 r。",
      "一般方程化标准方程常用配方。"
  ...
  },
  {
    ...
    "explanation": "导数可以理解为函数在某一点附近变化快慢，也可以看作切线斜率。基础阶段先把常见求导公式用熟。",
    "keyPoints": [
      "(x^n)' = nx^(n-1)。",
  ...
  },
  {
    ...
    "explanation": "导数判断单调性的核心是看 f'(x) 的正负。f'(x)>0 时函数递增，f'(x)<0 时函数递减。",
    "keyPoints": [
      "先求定义域，再求导数。",
  ...
  },
  {
    ...
    "reviewDate": "",
    "prerequisites": [],
    "explanation": "古典概型要求每个基本结果等可能。概率等于有利结果数除以所有可能结果数。",
    "keyPoints": [
